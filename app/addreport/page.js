...
const AddReport = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    refBy: "",
    address: "",
    tests: [{ testName: "", testCode: "", price: 0, status: "Pending" }],
  });
  ...
  const removeTest = (index) => {
    if (formData.tests.length > 1) {
      ...
      const cleanedSearchTerms = {};
      const cleanedDropdownOpen = {};
      Object.keys(newSearchTerms).forEach((key) => {
        const keyIndex = parseInt(key);
        if (keyIndex > index) {
          cleanedSearchTerms[keyIndex - 1] = newSearchTerms[key];
          cleanedDropdownOpen[keyIndex - 1] = newDropdownOpen[key];
        } else if (keyIndex < index) {
          cleanedSearchTerms[keyIndex] = newSearchTerms[key];
          cleanedDropdownOpen[keyIndex] = newDropdownOpen[key];
        }
      });

      setSearchTerms(cleanedSearchTerms);
      setDropdownOpen(cleanedDropdownOpen);
    }
  };
          ...
          <div className="space-y-8">
                ...
                {formData.tests.map((test, index) => (
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      {/* Test Name Dropdown with Search */}
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-slate-200">
                          Test Name *
                        </label>
                        <div className="relative">
                          {/* Search Input */}
                          <div className="relative">
                            <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 z-10" />
                            <input
                              type="text"
                              ...
                              className="w-full pl-10 pr-10 py-3 bg-white/5 border border-white/20 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300"
                              placeholder={
                                test.testName || "Search and select test..."
                              }
                            />
                            <button
                              type="button"
                          ...
                          {/* Dropdown List */}
                          {dropdownOpen[index] && (
                            <div className="absolute z-20 w-full mt-1 bg-slate-800 border border-white/20 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                              {getFilteredTests(index).length > 0 ? (
                                getFilteredTests(index).map(
                                  (testOption, testIndex) => (
                                    <button
                                      key={testIndex}
                                      type="button"
                                      onClick={() =>
            ...
            <div className="flex justify-center relative">
              <button
                type="button"
                onClick={addTest}
                className="fixed bottom-8 right-8 inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-300 transform hover:scale-105 shadow-lg z-10"
              >
                <FaPlus className="text-sm" />
                Add Test
              </button>
