...
const FindViaDoctor = () => {
  ...
  return (
    <div className="min-h-screen pt-32 bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="backdrop-blur-xl bg-white/10 rounded-2xl shadow-2xl border border-white/20 p-6 mb-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent flex items-center gap-3">
                <FaUserMd className="text-blue-400 text-3xl" />
                Find Reports by Doctor
              </h1>
              <p className="text-blue-200 mt-2 text-lg">
                Search and filter medical reports by referring doctor
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-12">
            ...
            <div className="relative group">
              <FaSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-blue-400 z-10" />
              <input
                type="text"
                placeholder="Search doctor name..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-12 pr-4 py-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-transparent text-white placeholder-blue-200 transition-all duration-300 group-hover:bg-white/15"
              />
              {searching && (
                <FaSpinner className="absolute right-4 top-1/2 transform -translate-y-1/2 text-blue-400 animate-spin" />
              )}
            </div>
        ...
        {!searchTerm && !selectedDoctor ? (
          <div className="backdrop-blur-xl bg-white/5 rounded-2xl shadow-2xl border border-white/10 p-16 text-center">
            <div className="relative">
              <FaUserMd className="text-8xl text-blue-400/30 mx-auto mb-6" />
          ...
          <div className="grid gap-6">
            {filteredReports.map((report) => (
              <div
                ...
                className="backdrop-blur-xl bg-gradient-to-r from-white/10 to-white/5 rounded-2xl shadow-2xl border border-white/20 p-6 hover:shadow-3xl transition-all duration-300 cursor-pointer border-l-4 border-l-cyan-400 group hover:scale-[1.02] hover:bg-gradient-to-r hover:from-white/15 hover:to-white/10"
              >
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                  {/* Patient Info */}
                  <div className="flex-1">
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 bg-gradient-to-br from-blue-400 to-cyan-500 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                  ...
                  <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
                    ...
                    <button className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 transform hover:scale-105 shadow-lg font-semibold">
                      <FaEye />
                      View Report
                    </button>
                  </div>
                ...
                <div className="mt-6 pt-6 border-t border-white/20">
                  <div className="flex flex-wrap gap-3">
                    {report.tests.slice(0, 3).map((test, index) => (
                      <span
                        key={index}
                        className="px-4 py-2 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 text-blue-200 rounded-full text-sm font-medium border border-blue-400/30 backdrop-blur-sm"
                      >
                        {test.testName}
                      </span>
                    ))}
