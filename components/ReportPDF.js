...
const ReportPDF = () => {
  ...
  const isOutOfRange = (value, referenceRange) => {
    if (!value || !referenceRange || referenceRange === "") return false;
    const numericValue = parseFloat(value);
    if (isNaN(numericValue)) return false;

    // Handle ranges like "upto X"
    if (referenceRange.startsWith("upto ")) {
      const max = parseFloat(referenceRange.replace("upto ", ""));
      return numericValue > max;
    }

    // Handle ranges like "<X"
    if (referenceRange.startsWith("<")) {
      const max = parseFloat(referenceRange.replace("<", ""));
      return numericValue >= max;
    }

    // Handle ranges like "X-Y"
    ...
    return (
      <div className="mb-8" key={testCode}>
        ...
        {parameters.map((param, idx) => {
          ...
          const outOfRange = isOutOfRange(value, param.referenceRange);
      ...
      <div className="max-w-4xl mx-auto print:mt-0">
        ...
        <div className="bg-white shadow-lg border relative border-gray-300 print:shadow-none print:border-0">
          <div className="absolute top-8 -right-20 translate-x-20">
            {showHeader ? (
              <button onClick={() => setShowHeader(false)} className="border px-4 py-2 rounded-full shadow-xl cursor-pointer text-white bg-gradient-to-tl to-violet-500 from-blue-500" >Hide Header</button>
            ) : (
              <button onClick={() => setShowHeader(true)} className="border px-4 py-2 rounded-full shadow-xl cursor-pointer text-white bg-gradient-to-tl to-violet-500 from-blue-500">Show Header</button>
            )}
          </div>
          {showHeader ? (
