...
import { FaPrint, FaSave, FaSpinner, FaCalculator, FaReceipt } from "react-icons/fa";
...
const GenerateBillComponent = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.existingBill) {
          ...
          setFinalAmount(data.existingBill.finalAmount || data.patient.totalPrice);
        } else {
  ...
  useEffect(() => {
    if (patient) {
      ...
      const percentOff = percentDiscount > 0 ? (amount * percentDiscount) / 100 : 0;
  ...
  if (!patient) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
          <h1 className="text-lg font-semibold text-slate-800 mb-4">No Patient Found</h1>
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-blue-500 text-white text-sm rounded-lg hover:bg-blue-600 transition-colors"
          >
        ...
        <div className="flex-1 print:max-w-none">
          <div className="bg-white shadow-xl print:shadow-none rounded-2xl print:rounded-none overflow-hidden border border-slate-200 print:border-none">
            ...
            <div className="p-4 print:p-3 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                  ...
                  {patient.refBy && (
                    <div className="bg-slate-50 p-2 rounded">
                      ...
                      <span className="font-medium text-slate-800">{patient.refBy}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Tests Section */}
              <div>
                <h3 className="text-sm font-semibold text-slate-800 mb-3">Tests Conducted</h3>
                <div className="bg-slate-50 p-3 rounded ">
                  ...
                  <p className="text-xs text-slate-600">
                    Total Tests: <span className="font-semibold">{patient.tests.length}</span>
                  </p>
                </div>
              </div>
              ...
              {/* Billing Summary */}
              <div>
                <h3 className="text-sm font-semibold text-slate-800 mb-3">Billing Summary</h3>
                <div className="bg-slate-50 p-3 rounded ">
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-600">Subtotal:</span>
                      <span className="font-semibold">₹{patient.totalPrice.toFixed(2)}</span>
                    </div>
                    ...
                    {percentDiscount > 0 && (
                      <div className="flex justify-between text-red-600">
                        ...
                        <span>-₹{(patient.totalPrice * (percentDiscount / 100)).toFixed(2)}</span>
                      </div>
                    )}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setRupeesDiscount(parseFloat(e.target.value) || 0)}
